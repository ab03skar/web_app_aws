#!/bin/bash
#script to autmate the cloudformation templates,
# Parameters
#   $1 is for actions, Valid values: deploy, delete, preview.
#   $2 the selected region.
...
if [[ $1 != "deploy" && $1 != "delete" && $1 != "preview" ]]; then
    echo "ERROR: Not a Valid input. Valid values are: deploy, delete, preview." >&2
    exit 1
fi
